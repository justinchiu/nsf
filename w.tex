...
which had modelled the text generation process through a hierarchical HSMM.
...
however we interested in the \textbf{posterior} distribution $p(\bc\mid\bx,\by)$ of
...
separately training 
% This was accomplished in \citet{wiseman2017d2t,puduppully2018contentselection} by predicting t\mid e,v and aligning to a complete record in $\bx$
The second task B) is reconstructing values $v$ in the table $\bx$.
In particular, we would want to learn $p(\bv \mid \by,\be,\bt)$,
the distribution over all values given the summary, all entities, and all types.
...
% Break from previous work, introduce conditional distributions of structure and content

% Move desiderata before proposal, to demonstrate hole in research that we solve
% but write in paragraph form to hide holes?
% We can't really measure these as they are qualitative??????
% are these obvious?
...
% Maybe mention pipelined approach?
\begin{enumerate}
