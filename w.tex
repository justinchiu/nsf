...
We do so by defining a conditional model of the text,
then using that generative model to obtain a distribution
over quantities of interest to be extracted.

We begin by first defining the information extraction
system's distribution of interest, then the conditional model.
Let $\ba$ be a latent variable that represents the
...
which allows us to infer the alignments and values given the text, entities, and types.
\begin{comment}
Since we do not assume that we have supervision for $\bz$,
we cannot learn the information extraction system directly.
Insead, we use a conditional model 
...
We define the conditional model $p(\by,\bz\mid\bx)$,
with $\bz,\bx$ defined as above. with the following generative story:
\begin{enumerate}
\item Value Choice:
