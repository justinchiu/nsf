...
where $\bz = \set{\bc, \bv}$ and $\bx = \set{\be, \bt}$.
In the previous section, we defined $\bc$ to be a latent variable that represents the
...
This distribution $p(\bz\mid\by,\bx)$ is the information extraction system,
...
Insead, we must instead learn a conditional model 
$p(\by,\bz\mid\bx)$ since we do have supervision for $\by$.
