...
% Mention amortized variational inference in case of intractable exact inference?
...
and marginalizing over the alignment distribution allows uncertainty over alignments
...
We introduce a new step to the generative process of \texttt{Values} that 
allows the model to learn new records as functions of ones already defined in the data.
We may treat this as a program induction problem, where we would like to learn new relations
that are the result of boolean functions applied to exists records.
Alternatively, we may attempt to approximate a boolean function with neural networks directly.
(TODO: finish this?)
...
enormous growth make it clear that a method for training information extraction systems
