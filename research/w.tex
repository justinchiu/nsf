...
Figure~\ref{fig:boxscore} is an example of a datum and text pair which may be used to
...
are to the left of the summary in Figure~\ref{fig:boxscore}.
\end{enumerate}
...
\begin{enumerate}
\item A LVM can learn in the semi-supervised or unsupervised setting,
where only a few labels or no labels are provided respectively.
This implies we can train a model with missing or noisy facts.
...
Typically this involves applying Bayes' rule and total probability, then marginalizing
over latent quantities.
using the 
