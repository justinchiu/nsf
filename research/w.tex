...
\centering
 ...
 \begin{tikzpicture}[every node/.style={anchor=base,minimum size=8mm}]
    ...
    minimum width=0.2em, minimum height=1em, font=\small,ampersand replacement=\&] {
      \&$\subalign{e_1:& \textrm{ michael jordan}\\ t_1:& \textrm{ POINTS}\\ v_1:& ?}$\&
      \&$\subalign{e_2:& \textrm{ michael jordan}\\ t_2:& \textrm{ REBOUNDS}\\ v_2:& ?}$\&
      \&$\subalign{e_3:& \textrm{ michael jordan}\\ t_3:& \textrm{ BLOCKS}\\ v_3:& ?}$ \\
      $\ba$ \\
    ...
    \draw[draw=blue!30, line width=0.2mm] ($(graph-1-2) + (0.1, 0)$) -- ($(graph-3-3) + (0.1, 0)$);
    \draw[draw=blue!30, line width=0.2mm] ($(graph-1-4) + (0.1, 0)$) -- ($(graph-3-5) + (0.1, 0)$);
    \draw[draw=blue!30, line width=0.2mm] ($(graph-1-6) + (0.1, 0)$) -- ($(graph-3-7) + (0.1, 0)$);


    \draw[rounded corners,fill=red!10] ($ (graph-3-1.north west) +(-0.1,0.1)$) rectangle 
    node[yshift=-1cm]{} ($(graph-3-7.south east) +(0.1,-0.1)$ );
    \draw[rounded corners,fill=red!10] ($ (graph-1-2.north west) +(-0.1,0.1)$) rectangle  
    node[yshift=0.8cm]{} ($(graph-1-6.south east) +(0.1,-0.1)$ );

    \draw[rounded corners,fill=green!10] (graph-1-2.north west) rectangle  node[ yshift =0.9cm] {$r_1$} (graph-1-2.south east);
    \draw[rounded corners,fill=green!10] (graph-1-4.north west) rectangle  node[ yshift =0.9cm] {$r_2$} (graph-1-4.south east);
    \draw[rounded corners,fill=green!10] (graph-1-6.north west) rectangle  node[ yshift =0.9cm] {$r_3$} (graph-1-6.south east);

    \draw[rounded corners, fill=blue!10] (graph-3-3.north west) rectangle 
    node[yshift=-0.9cm]{$\by_{4:5}$}  ($(graph-3-3.south east) + (-0.1, 0)$);
    \draw[rounded corners, fill=blue!10] (graph-3-5.north west) rectangle 
    node[yshift=-0.9cm]{$\by_{7:8}$}  ($(graph-3-5.south east) + (-0.1, 0)$);
    \draw[rounded corners, fill=blue!10] (graph-3-7.north west) rectangle 
    node[yshift=-0.9cm]{$\by_{12:13}$}  ($(graph-3-7.south east) + (-0.1, 0)$);
    \end{scope}
  \end{tikzpicture}
...
Considering the first record $r_1$, we want to infer the value $v_1$ given 
...
In this case we have $\by_{4:5}$ are both aligned to $r_1$, and thus
$a_4 = 1$ and $a_5 = 1$ indicating that the fourth and fifth
words are aligned to the first record.
...
\item Move towards an end-to-end system for knowledge graph completion.
\end{enumerate}
...
we choose a sequence of records $\ba = \set{a_1,\ldots,a_T}$ to describe.
(Need to redo example for HSMM?)
