...
\begin{enumerate}
\item First segment the text into mentions and values.
\item Then align the mentions to a entity, a node in a knowledge graph.
\item Finally identify the relationships between segments, for example
whether two entities are on the same sports team.
The relationships between these segments entail facts.
\end{enumerate}
Figure~\ref{fig:boxscore} is an example of a datum which may be used to
train an information extraction system.
The example is inspired by the Rotowire dataset \citep{wiseman2017d2t},
which contains summaries of basketball games paired with player and team statistics,
which we refer to as data.
In this example, an information extraction system would segment the summary;
identify all players, teams, and numerical or geographic values;
predict the relationships between those segments;
and finally perform extraction by filling in the table of statistics
with tuples of entities, relation types, and values.

Given the large cost of obtaining labels to train an extraction system,
which would entail manually labeling entities, values, and their relationships with one another,
...
learning to recreate the text is much easier than using the no
given facts to train an extraction system.
Such a model of text is called a generative model, which in the information extraction setting
learns to generate the facts and then the text conditioned on the facts. 
A generative model is defined by its generative process, which 
is a recipe that describes how all quantities of interested are created.

The benefit of using a generative model rather than directly modeling facts given text is three-fold:
A generative model can learn in the semi-supervised or unsupervised setting,
where only a few labels or no labels are provided respectively.
This implies we can train a model without observing all facts, or even with noisy facts.
Secondly, the ability to learn without observing extends not only to the 
final quantities of interest, but also intermediate ones.
A generative process may include latent or unobserved variables
that can serve to add intepretable structure the model,
for example by learning to the relationship between segments of text and
the records that generate them.
A generative model that includes latent variables is known as a latent variable model (LVM),
which will be the main focus of this proposal.
Finally, generative models provide a principled approach to inverting their generative process,
allowing one to reason about unobserved quantities earlier in the process given
observations later in the process. 
This is accomplished through belief propagation, an algorithm for
using the structure of the model to infer missing values.
...
from the text. The inferred values are in curly braces.}
\label{fig:d2t}}
...
\item Introduction
    ...
    \item Information Extraction at a high level
        ...
        \item Define information extraction
            ...
            \end{enumerate}
        \item LVM for sample complexity and semi-supervised training.
            Sample complexity comes from having a generative model that we can invert.
            Given a scarcity of labels in the semi-supervised regime,
            specifically where thereis more signal in generating the text,
            having a generative model that we can invert through posterior inference
            provides a principled approach to information extraction.
