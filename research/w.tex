...
{\begin{tikzpicture}[every node/.style={anchor=base,minimum size=8mm}]
   \matrix (kb) [matrix of nodes, row sep=0.5em,column sep=1.5em,
    ...
    \matrix (txt) [below=of kb,matrix of nodes, row sep=0.5em,column sep=0.05em,
    ...
    \draw[rounded corners, fill=blue!10] (txt-1-1.north west)+(.05,0) rectangle 
    node[yshift=-0.9cm]{$\by_{1:3}$}  ($(txt-1-3.south east) + (-.05, 0)$);
    \draw[rounded corners, fill=blue!10] (txt-1-4.north west)+(.05,0) rectangle 
    node[yshift=-0.9cm]{$\by_{4:5}$}  ($(txt-1-5.south east) + (-.05, 0)$);
    \draw[rounded corners, fill=blue!10] (txt-1-6.north west)+(.05,0) rectangle 
    node[yshift=-0.9cm]{$\by_{6:8}$}  ($(txt-1-8.south east) + (-.05, 0)$);
    \draw[rounded corners, fill=blue!10] (txt-1-9.north west)+(.05,0) rectangle 
    node[yshift=-0.9cm]{$\by_{9:13}$}  ($(txt-1-13.south east) + (-.05, 0)$);
    \end{scope}
\end{tikzpicture}}
\end{figure}
