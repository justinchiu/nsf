...
knowledge graph conditioned on text.
...
Each text $\by = y_1,y_2,\ldots$ is a sequence of tokens each from a vocabulary $V$. As an example consider a dataset of assigned summaries of basketball games $\by$ aligned with the respective statistics $\br$ of those games in Figure~\ref{fig:d2t}. 
...
\matrix (txt) [matrix of nodes, row sep=0.5em,column sep=0.05em,
   ...
   \matrix (kb) [below=of txt,matrix of nodes, row sep=0.5em,column sep=1.5em,
    ...
    \draw[line width=0.5mm] ($(kb-1-4) + (0.1, 0)$) -- node[left] {$a_3$} ($(txt-1-7) + (0.1, 0)$);
    \draw[line width=0.5mm] ($(kb-1-3) + (0.1, 0)$) -- node[right] {$a_4$} ($(txt-1-11) + (0.1, 0)$);
...
\end{tikzpicture}}
\end{figure}
My proposed model is a conditional LVM that specifies the relationship between data, specifically how the entities and types generate the text. Assume that the entities and relation types $\set{\be,\bt}$ of the knowledge graph are known. The model requires a generative process for
...
I plan to develop a system based on these ideas for information extraction with distant supervision. I will evaluate the approach on reduced annotation version of standard knowledge graph extraction benchmarks including, TAC KBP 2015 slot filling and  TACRED \citep{zhang2017slotfilling}.
