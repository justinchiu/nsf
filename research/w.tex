...
recent approaches make progress by fitting large models large datasets \citep{lee2018coref,zhang2017slotfilling}.
...
Latent variable models (LVMs) in particular lend themselves well to sample efficiency and semi-supervised learning.
...
which indicates that the expressivity of neural networks can be combined with the LVM framework
in order to train with less supervision.
In machine translation, \citet{deng2018attn} demonstrate that formulating attention as
...
as latent random variables and deal with them in a probabilistically principled fashion.
We will leverage recent advances in deep generative modeling in order to
train without explicit supervision.
\citep{surdeanu2012miml}
\citep{liang2009semalign}
